...
- **Window-target /screenshot returned occluding window content via BitBlt — RESOLVED s26** (session-25 discovery, session-26 P0 RESOLVED, constraint #46): the original `screenshot-helper.ps1` window mode used `Graphics.CopyFromScreen` at the bridge-supplied rect — captured whatever pixels were visible at those screen coordinates, NOT the target window's own backbuffer. Whenever another window (opencode terminal, VS Code, browser) was on top, the capture returned the OCCLUDING window's content with the VW window's bounding box. Reproduced live in s25 against all 4 VW windows (GemStone Launcher / Workspace / MAS Loaded Items / storedev64 launcher) — every capture returned opencode terminal text instead of VW window content (5 baseline PNGs preserved at `tm/screenshots/0[0-4]-*.png`). **RESOLVED in s26** (`e017d72` on `ENVIRO365/vw-runtime-api/main`): helper now filters HWND candidates to `vwnt.exe` PIDs via `Get-Process` + `GetWindowThreadProcessId` (eliminates false positives so no fixed rect tolerance is needed), scores remaining HWNDs by Manhattan edge delta vs the bridge-supplied rect, sizes the bitmap to the actual visible frame via `DwmGetWindowAttribute(DWMWA_EXTENDED_FRAME_BOUNDS)` (excludes 8 px DWM invisible resize border, includes title bar), and calls `PrintWindow(hwnd, hdc, PW_RENDERFULLCONTENT=0x00000002)` to render the window's own DC backbuffer regardless of z-order. PW_RENDERFULLCONTENT (Windows 8.1+) ensures DWM-composited content is captured for both classic GDI windows (VisualWorks) and modern DirectComposition apps. Falls back to legacy `CopyFromScreen` + WARN to stderr if no vwnt HWND matches or `PrintWindow` returns FALSE — so the helper never returns 0 bytes for a known-good rect. Screen-mode behavior preserved (still `CopyFromScreen` of `SystemInformation.VirtualScreen`). **Diagnostic key — VW rect convention**: VW's `windowsSnapshotForWait` reports the CLIENT area (origin below title bar): L/R/B match `DwmExtFrame` within ±1 px, T is offset by the title bar height (~31 px on Win10/11). Tested live by enumerating all 4 VW HWNDs + comparing each to VW's reported rect. **No bridge restart needed**: `OS.ExternalProcess execute:arguments:` in `captureScreenshotViaSubprocess:rect:` passes `-File <helperPath>` to a fresh PowerShell subprocess on every `/screenshot` call — the helper script is read from disk per-invocation. Implication: future helper edits land on the next call without `Start-VWBridge.ps1 -KillExisting`. Verified s26 against all 4 live VW windows — every capture now shows actual window content (menus, code editors, bundle trees) despite opencode being on top. Post-fix PNGs at `tm/screenshots/s26-fix-0[0-4]-*.png`. Smoke test step 24 (`vw_screenshot full-screen returns valid PNG ImageContent`) locks the happy path.
- **`applyErrorMapToShape` mutated only top-level field schemas — nested errors fell through to zod's default — RESOLVED s26** (session-25 discovery, session-26 RESOLVED, constraint #47): the original `applyErrorMapToShape` walked the tool-input `ZodRawShape` and set `_def.errorMap = mcpErrorMap` on each top-level field. Worked for direct-field errors (`Missing required parameter 'confirm'`) but NOT for nested errors. Nested validation paths like `windowSpec.components.0.type` are emitted by inner `ZodArray` / `ZodObject` / `ZodEnum` validators that carried no errorMap, so they hit zod's default `'Invalid enum value...'` instead of the actionable `Parameter 'windowSpec.components.0.type' must be one of [Label, ..., DataSet], but received 'BogusWidget'.` envelope. Verified live in s25 via `vw_create_window_spec` with `type: 'BogusWidget'`. **RESOLVED in s26** (`1bd6cf7` tm-context local): new `applyErrorMapToSchema` recursive walker descends every reachable Zod child schema by `_def.typeName`, attaching the errorMap to `ZodObject` shape fields, `ZodArray` element, `ZodTuple` items + rest, `ZodUnion` / `ZodDiscriminatedUnion` options, `ZodIntersection` left + right, `ZodOptional` / `Nullable` / `Default` / `Readonly` / `Branded` / `Catch` innerType, `ZodEffects` schema, `ZodRecord` / `ZodMap` / `ZodSet` key/value, `ZodLazy` getter result (with WeakSet cycle guard for self-referential schemas), `ZodPipeline` in + out, and `ZodPromise` type. Primitive leaf types (`ZodString`, `ZodNumber`, `ZodBoolean`, `ZodEnum`, etc.) just get the map and don't recurse. `applyErrorMapToShape` now routes each top-level field through the recursive walker, sharing a WeakSet across the whole shape so a cross-field DAG doesn't get visited twice. **Still per-schema, NOT global**: setting `z.setErrorMap` globally would rewrite the MCP SDK's OWN internal JSON-RPC parsing errors and break protocol handling (manifests as request hangs). The walker only touches schemas reachable from tool-input shapes registered via `withMcpErrorMap(server)`. 14 new tests added in `mcpZodErrorMap.test.ts` covering every wrapper type + the canonical `components.0.type` enum integration case + `components.0.name` invalid_type + `components.0.required` missing-required. vitest count net: 345 → 359.
- **Scaffolder emitted bare-leaf compile receiver — only namespace=Smalltalk worked — RESOLVED s27** (session-26 discovery, session-27 RESOLVED, constraint #48): `vw_create_application_model` + `vw_create_dialog` threaded `namespace` through to step 1 (`Smalltalk.<NS> defineClass:` via `emitCreateClassSmalltalk`) but NOT to the subsequent compile steps (aspect / action / hook / windowSpec). `buildInstanceCompileExpression` (applicationModel.ts:88) + `buildCompile` (dialog.ts:64) emitted `<className> compile: ... classified: ...` with the BARE LEAF as receiver. Works for namespace=Smalltalk because the bareword auto-resolves through the Smalltalk SystemDictionary; for every other namespace (Examples, GemStoneClasses, ...) the receiver evaluates to nil and the `compile:` keyword fans out into MNU `#compile:classified:`. Step 1 succeeds (class lands in the right namespace) but step 2 fails — observed live in s26 attempting MAS-side demo placement with `namespace=GemStoneClasses` then `namespace=Examples` (both produced identical MNU after the defineClass: 8-kw step succeeded). Memory entity: `Scaffolder-unqualified-className-receiver-bug`. **RESOLVED in s27** (tm-context commit): both helpers now take `namespace` as their first argument and always emit FQN receiver `${namespace}.${className}` (or `${namespace}.${className} class` for class-side methods like `windowSpec`). `namespace=Smalltalk` emits `Smalltalk.X compile:` which resolves identically to bare `X compile:` but keeps the emission shape consistent across namespaces (no conditional — smallest correct change). 6 new vitest cases lock the FQN receiver in `applicationModel.test.ts` + `dialog.test.ts` covering `namespace=Smalltalk` consistency + `namespace=Examples` (the bug) + `namespace=GemStoneClasses` across aspect/action/hook/windowSpec compile steps with positive (`/Examples\.Foo (class )?compile:/`) and negative (`/(?<![\w.])Foo (class )?compile:/`) matchers so a future regression cannot slip back to the bare-leaf form. 3 existing test assertions updated to expect FQN form (PartySearchView in MyAppPackage, TableViewApp in MyApp DataSet, RowPickerDialog in MyApp). vitest count net: 359 → 365. Smoke test step 28 (`vw_create_application_model namespace=Examples`) locks the end-to-end behavior against the live bridge — pre-flight verifies `Examples` namespace exists (fails loud if not, so the test never silently skips); happy path creates `Examples.SmokeTestNonSmalltalkNS_<tag>` with `note` ivar + `windowSpec`; cleanup via `vw_eval (Examples at: #cls ifAbsent: [nil]) ifNotNil: [:c | c removeFromSystem]` because `vw_delete_class` still calls `<className> removeFromSystem` with bare leaf (potential future bug, deferred — surfaced as the smoke step's cleanup-path workaround). **Standalone tools unchanged**: `vw_compile_method` / `vw_define_action` / `vw_define_aspect` accept `className` from user input which the schema docs note may be "Single segment or namespace-qualified"; for non-Smalltalk classes the caller MUST pass FQN. The scaffolders had this bug specifically because they threaded `namespace` separately from `className` and only the `defineClass:` path used the combination.
