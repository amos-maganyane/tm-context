...
# VisualWorks 9.3.1 Code-Distribution Mechanism � Programmatic API Reference

> **Scope**: This document inventories every code-distribution API a VisualWorks
> developer would use programmatically � Pundle, Parcel, Bundle, Package, Store.
> It is written for **Phase M of the mcp-vw server** and is meant to back the
> native-typed MCP tool surface that AI clients will call. Every claim is sourced
> to either (a) literal Smalltalk from VW/StoreCI parcels, (b) Cincom's own
> 7.6/8.0/9.x guides (parcels-paper + AppDevGuide + SourceCodeMgmtGuide), or
